...
2. **`./private/openspec`** — detailed author specification (look at all `design.md`, `proposal.md`, and `spec.md` anywhere in the file tree plus any doc directly at L1 of the tree).
...
- the `code-style` and `architecture` skills
- `docs/adrs/` (architectural decisions are standards)
- `CONTRIBUTING.md`
- `.oxlintrc`, `.oxfmtrc`, `.editorconfig`, `tsconfig.json` (machine-enforced standards — note them but don't re-check what tooling already checks)
